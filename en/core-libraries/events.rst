...
do it. So we need another solution, let's rewrite that using the event manager::
    ...
    class Order extends AppModel {
        ...
        public function place($order) {
            if ($this->save($order)) {
                $this->Cart->remove($order);
                $this->getEventManager()->dispatch(new Event('Model.Order.afterPlace', $this, array(
                    'order' => $order
                )));
                return true;
            }
...
the ``EventListener`` interface. Internally, the event manager will read the
...
consequence depends on the code triggering the event, for instance, in our
``afterPlace`` example it would not make any sense to cancel the operation since
the data was already saved and the cart emptied. Nevertheless, if we had
a ``beforePlace`` stopping the event would have a valid meaning, and could be
used to prevent the operation from occuring. This pattern is used throughout
CakePHP.

To check if an event was stopped, you call the ``isStopped()`` method in the event
object::
...
params you used for attaching it::
    ...
    $listener = new MyCakeEventListner();
...
The global manager is a singleton instance of a ``EventManager`` class that
...
the following example will attach a global event to the ``beforePlace`` event::
    ...
    EventManager::instance()->attach($aCallback, 'Model.Order.beforePlace');
...
reality, it cannot do its logic if the model is the Cart::
    ...
    EventManager::instance()->attach('myCallback', 'Model.beforeFind');
